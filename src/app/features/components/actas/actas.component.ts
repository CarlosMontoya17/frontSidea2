...
import {
  actionEmitter,
  cardFilter,
} from 'src/app/shared/models/card-filters.model';
import { cardInfo } from 'src/app/shared/models/card-information.model';
import {
  cardRequest,
  searchCURP,
} from 'src/app/shared/models/card-request.model';
...
export class ActasComponent implements OnInit {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      if (e.search == 'CURP') {
        ...
        const _d: cardRequest = {
          ...
          Index: i + 1,
          Id: e.id,
          Background: `/assets/images/icons/${String(
            e.type
          ).toLowerCase()}.png`,
          Date: e.createdAt,
  ...
  onDownloadActa(Id: any, CURP: any): void {
    this.svc.downloadActa(Id).subscribe(
      (data: any) => {
        if (data?.b64) {
